...
class ColumnKeyOptionTest < Minitest::Test
  ...
  def test_default_predefined_ranges
    ...
    expected = {
      ...
      "data-datatable-predefined-ranges" => {Today: [predefined_ranges[:datetime][:Today].first.to_s,
        predefined_ranges[:datetime][:Today].last.to_s]}
    }
  ...
  def test_column_predefined_ranges
    ...
    expected = {
      "data-datatable-range" => :datetime,
      "data-datatable-predefined-ranges" => {Today: [predefined_ranges[:Today].first.to_s,
        predefined_ranges[:Today].last.to_s]}
    }
    assert_equal expected, column_key_options[0][:html_options]
    expected = {
