...
class DtParamsTest < Minitest::Test
  ...
  def test_raise_exception_if_global_search_is_not_correct_format
    ...
    assert_equal 'TrkDatatables: String does not have #dig method. Global search is in a format: { "search": { "value": "ABC" } }',
      e.message
  end
  ...
  def test_raise_exception_if_column_search_is_not_correct_format
    ...
    assert_equal 'TrkDatatables: String does not have #dig method. Column search is in a format: { "columns": { "0": { "search": { "value": { "ABC" } } } } }',
      e.message
  end
end
